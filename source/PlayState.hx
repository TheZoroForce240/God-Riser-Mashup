...
class PlayState extends MusicBeatState
{
	...
	private var redshaggy:Character;
	...
	override public function create()
	{
		...
		else if (SONG.song.toLowerCase() == 'god-riser')
		{
			defaultCamZoom = 0.65;
			curStage = 'sky';
		...
		redshaggy = new Character(0, 0, 'redpshaggy');
	...
	function startCountdown():Void
	{
		...
		if (SONG.song.toLowerCase() == "god-riser")
		{
			new FlxTimer().start(0.002, function(cbt:FlxTimer)
			{
				if (ctrTime == 0)
			...
			new FlxTimer().start(2, function(redshag:FlxTimer)
			{
				FlxG.sound.play(Paths.sound('shagFly'));
				FlxTween.tween(redshaggy, {x: (dad.x - 300), y: (dad.y)}, 0.5);

			});
			new FlxTimer().start(3, function(redshagflytmr:FlxTimer)
				{
					redshagfly = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null && !songEnded)
		{
			...
			{
				if (isredshaggy == true)
					camFollow.setPosition(redshaggy.getMidpoint().x + 150, dad.getMidpoint().y - 100);

				else if (isredshaggy == false)
				{
							...
							if (mania == 0)
								{
									...
									{
										case 2:
											redshaggy.playAnim('singUP' + altAnim, true);
										case 3:
											redshaggy.playAnim('singRIGHT' + altAnim, true);
										case 1:
											redshaggy.playAnim('singDOWN' + altAnim, true);
										case 0:
											redshaggy.playAnim('singLEFT' + altAnim, true);
									}
								}
								else if (mania == 1)
								{
